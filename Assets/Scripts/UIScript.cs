...
public class UIScript : MonoBehaviour
{
    ...
    void Update()
    {
            ...
            if (Input.GetKeyDown(KeyCode.Alpha1))
            {
                if (Indicator.isOnCraftingTableUI)
                {
                    if (Indicator.gravestone >= Indicator.weaponcost)
                    {
                        ...
                        Indicator.gravestone -= Indicator.weaponcost;
        ...
        if (type == "damageindicator")
        {
            if (Indicator.damageTime <= 1f && Indicator.damageTime >= .7f && Indicator.gotDamaged)
            {
                spritemeter.enabled = true;
            }
            else {
                spritemeter.enabled = false;
            }
        }
        if (type == "pauseui")
        {
